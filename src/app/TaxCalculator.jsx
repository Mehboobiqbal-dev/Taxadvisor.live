...
// Flat state tax rates for most states (approximation)
...
const TaxCalculator = () => {
  ...
  const calculateTaxes = () => {
    ...
    // Parse numerical inputs (defaulting to 0 if blank)
    ...
    // Calculate federal tax on ordinary income using federal brackets
    const federalTaxOrdinary = calculateTaxFromBrackets(taxableOrdinaryIncome, federalTaxBrackets);
    // Calculate preferential tax on qualified dividends and LT gains
    ...
    // Calculate state tax:
    // If California, use progressive brackets; otherwise, use flat rate from statesTaxRates.
    let stateTax = 0;
    if (state === 'CA') {
